...
(function() {
  ...
  function renderReports(orders) {
    ...
    for (var i = 0; i < orders.length; i++) {
      ...
      var buttons = '';
      if (isCompleted) {
        buttons = '<a href="/api/reports/' + o.id + '/html" target="_blank" class="px-4 py-2 bg-brand-600 text-white rounded-lg text-sm font-medium hover:bg-brand-700"><i class="fas fa-file-alt mr-1"></i>View Report</a><a href="/api/reports/' + o.id + '/pdf" target="_blank" title="Opens print dialog in new tab" class="px-4 py-2 bg-white/5 text-gray-300 rounded-lg text-xs font-medium hover:bg-gray-200"><i class="fas fa-print mr-1"></i>Print PDF</a><a href="/api/reports/' + o.id + '/customer-html" target="_blank" title="Aerial &amp; diagrams only, no measurements" class="px-4 py-2 bg-emerald-600 text-white rounded-lg text-xs font-medium hover:bg-emerald-700"><i class="fas fa-user mr-1"></i>Customer Copy</a><a href="/api/reports/' + o.id + '/customer-pdf" target="_blank" title="Print the customer copy" class="px-4 py-2 bg-white/5 text-gray-300 rounded-lg text-xs font-medium hover:bg-gray-200"><i class="fas fa-print mr-1"></i>Customer PDF</a><a href="/api/reports/' + o.id + '/customer-pdf?save=1" target="_blank" title="Download customer copy as PDF" class="px-4 py-2 bg-emerald-700 text-white rounded-lg text-xs font-medium hover:bg-emerald-800"><i class="fas fa-download mr-1"></i>Save PDF</a><button onclick="window._crmShareReport(' + o.id + ')" class="px-4 py-2 bg-sky-600 text-white rounded-lg text-xs font-medium hover:bg-sky-700"><i class="fas fa-share-alt mr-1"></i>Share</button><button onclick="window._crmCreateProposalFromReport(' + o.id + ')" class="px-4 py-2 bg-blue-500/15 text-white rounded-lg text-xs font-medium hover:bg-blue-500/15"><i class="fas fa-file-invoice mr-1"></i>Create Proposal</button>';
        if (isSolar) {
