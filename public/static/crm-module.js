  ...
  function renderInvoices(data) {
    ...
    if (invoices.length === 0) {
      ...
      for (var i = 0; i < invoices.length; i++) {
        var inv = invoices[i];
        html += '<tr class="hover:bg-white/5 cursor-pointer" onclick="window._crmViewInvoice(' + inv.id + ')"><td class="px-4 py-3 font-mono text-xs font-bold text-white">' + inv.invoice_number + '</td><td class="px-4 py-3 text-white">' + (inv.customer_name || 'N/A') + '</td><td class="px-4 py-3 text-gray-300 text-xs hidden md:table-cell">' + fmtDate(inv.created_at) + '</td><td class="px-4 py-3 text-gray-300 text-xs hidden md:table-cell">' + fmtDate(inv.due_date) + '</td><td class="px-4 py-3 text-center">' + badge(inv.status) + '</td><td class="px-4 py-3 text-right font-semibold text-white">' + money(inv.total) + '</td><td class="px-4 py-3 text-right" onclick="event.stopPropagation()"><div class="flex items-center gap-1 justify-end">';
        html += '<button onclick="window._crmViewInvoice(' + inv.id + ')" class="text-xs text-white hover:underline font-medium"><i class="fas fa-eye mr-0.5"></i>View</button>';
        if (inv.status === 'draft') html += '<button onclick="window._crmEditInvoice(' + inv.id + ')" class="text-xs text-gray-200 hover:underline ml-2"><i class="fas fa-edit mr-0.5"></i>Edit</button>';
        if (inv.status === 'draft' || inv.status === 'sent') html += '<button onclick="window._crmSendInvoice(' + inv.id + ')" class="text-xs text-blue-400 hover:underline ml-2"><i class="fas fa-paper-plane mr-0.5"></i>Send</button>';
        if (inv.square_payment_link_url) {
          html += '<a href="' + inv.square_payment_link_url + '" target="_blank" class="text-xs bg-emerald-500/15 text-emerald-400 px-2 py-0.5 rounded-full font-semibold ml-2 hover:bg-green-200"><i class="fas fa-credit-card mr-0.5"></i>Pay Link</a>';
        } else if (inv.status !== 'paid' && inv.status !== 'cancelled') {
          html += '<button onclick="event.stopPropagation();window._crmGenPayLink(' + inv.id + ')" class="text-xs bg-emerald-500/15 text-emerald-400 px-2 py-0.5 rounded-full font-semibold ml-2 hover:bg-emerald-200"><i class="fas fa-credit-card mr-0.5"></i>+ Pay Link</button>';
        }
        if (inv.status !== 'paid' && inv.status !== 'cancelled') html += '<button onclick="window._crmMarkInvoice(' + inv.id + ',\'paid\')" class="text-xs text-emerald-400 hover:underline ml-2">Mark Paid</button>';
