...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderSelectReport() {
    ...
    var searchBar = all.length > 0
      ? '<div style="margin-bottom:12px">' +
          '<input id="pb-report-search" type="search" enterkeyhint="search" placeholder="Search address\u2026" value="' + (term || '') + '" ' +
      ...
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">' +
        ...
        '<div>' +
              ...
              (function() {
                ...
                var priceInput = function(key, label, suffix) {
                  ...
                  return '<div><label style="color:var(--text-muted);font-size:11px;display:block;margin-bottom:2px">' + label + '</label>' +
                    ...
                    '<input type="number" inputmode="decimal" step="0.01" value="' + val + '" onchange="window._pb.setBundlePrice(\'' + key + '\',this.value)" style="width:100%;background:var(--bg-elevated);border:1px solid var(--border-color);border-radius:6px;padding:6px 8px;color:var(--text-primary);font-weight:700;font-size:13px">' +
                  ...
                  '<tbody>' +
                    items.map(function(item, i) {
                      return '<tr style="border-bottom:1px solid var(--border-color)">' +
                        ...
                        '<td style="text-align:center;padding:4px"><input type="number" inputmode="decimal" value="' + (item.quantity||1) + '" onchange="window._pb.updateItem(' + i + ',\'quantity\',this.value)" style="width:50px;background:transparent;border:1px solid var(--border-color);border-radius:4px;color:var(--text-primary);font-size:12px;text-align:center;padding:4px"></td>' +
                        '<td style="text-align:right;padding:4px"><input type="number" inputmode="decimal" value="' + (item.unit_price||0) + '" onchange="window._pb.updateItem(' + i + ',\'unit_price\',this.value)" style="width:80px;background:transparent;border:1px solid var(--border-color);border-radius:4px;color:var(--text-primary);font-size:12px;text-align:right;padding:4px"></td>' +
          ...
          '<div style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:12px;padding:16px">' +
            ...
            :
              '<div><label style="color:var(--text-muted);font-size:11px">Customer $/Square</label><input type="number" inputmode="decimal" value="' + (state.customerPricePerSquare || 0) + '" onchange="window.__pbState.customerPricePerSquare=Number(this.value);window.__pbRender()" style="width:100%;background:var(--bg-elevated);border:1px solid var(--border-color);border-radius:8px;padding:8px;color:var(--text-primary);font-weight:700;font-size:18px;margin-top:4px"></div>'
            ) +
          '</div>' +
          ...
          <tbody class="divide-y divide-gray-100">
            ${proposals.map(p => {
              ...
              return `<tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3"><span class="px-2 py-0.5 rounded-full text-xs font-medium ${sc}">${p.status}</span>${p.viewed_count > 0 ? '<span class="text-[10px] text-gray-500 ml-2" title="Viewed ' + p.viewed_count + ' times"><i class="fas fa-eye text-gray-600"></i> ' + p.viewed_count + '</span>' : ''}${(p.created_by === 'auto-invoice' && p.status === 'sent') ? '<span class="text-[10px] ml-2 px-2 py-0.5 rounded-full font-semibold" style="background:#ecfeff;color:#0e7490;border:1px solid #a5f3fc" title="Drafted and emailed automatically on report completion' + (p.sent_date ? ' (' + p.sent_date + ')' : '') + '"><i class="fas fa-robot" style="margin-right:3px"></i>Auto-sent</span>' : ''}${(p.created_by === 'auto-invoice' && p.status === 'draft') ? '<span class="text-[10px] ml-2 px-2 py-0.5 rounded-full font-semibold" style="background:#fef3c7;color:#92400e;border:1px solid #fde68a" title="Auto-drafted — email send did not complete; retry from actions"><i class="fas fa-robot" style="margin-right:3px"></i>Auto-draft · retry</span>' : ''}</td>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
      ...
      ` : `
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label class="block text-xs font-semibold text-gray-500 mb-1">Name *</label><input type="text" id="pb-nc-name" autocomplete="name" enterkeyhint="next" value="${f.newCustomer.name}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="John Smith"></div>
          <div><label class="block text-xs font-semibold text-gray-500 mb-1">Email *</label><input type="email" id="pb-nc-email" autocomplete="email" inputmode="email" enterkeyhint="next" value="${f.newCustomer.email}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="john@example.com"></div>
          <div><label class="block text-xs font-semibold text-gray-500 mb-1">Phone</label><input type="tel" id="pb-nc-phone" inputmode="tel" autocomplete="tel" enterkeyhint="next" value="${f.newCustomer.phone}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="(780) 555-1234"></div>
          <div><label class="block text-xs font-semibold text-gray-500 mb-1">Company</label><input type="text" id="pb-nc-company" autocomplete="organization" enterkeyhint="next" value="${f.newCustomer.company_name}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="Smith Roofing Ltd."></div>
          <div class="md:col-span-2"><label class="block text-xs font-semibold text-gray-500 mb-1">Address</label><input type="text" id="pb-nc-address" autocomplete="street-address" enterkeyhint="done" value="${f.newCustomer.address}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="123 Main St, Edmonton, AB"></div>
        </div>
      `}
    </div>
      ...
      '<div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">' +
        ...
        '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;text-align:center">' +
          (function() {
            ...
            return '<div style="background:white;border:1px solid #e5e7eb;border-radius:8px;padding:16px"><div style="color:#6b7280;font-size:11px;text-transform:uppercase;margin-bottom:4px">Roof Squares</div><input type="number" inputmode="decimal" value="' + edSq + '" oninput="window.__pbState.manualSquares=Number(this.value)||0;window._pb.updatePerSquare()" style="width:100%;background:transparent;border:none;border-bottom:2px solid #1a1a2e;color:#1a1a2e;font-size:28px;font-weight:800;text-align:center;padding:0" min="0" step="0.5"></div>' +
              '<div style="background:white;border:1px solid #e5e7eb;border-radius:8px;padding:16px"><div style="color:#6b7280;font-size:11px;text-transform:uppercase;margin-bottom:4px">Your Cost Per Square</div><div>$<input id="pps-price" type="number" inputmode="decimal" value="' + (state.pricePerSquare || 350) + '" oninput="window.__pbState.pricePerSquare=Number(this.value);window._pb.updatePerSquare()" style="width:80px;background:transparent;border:none;border-bottom:2px solid #2563eb;color:#1a1a2e;font-size:28px;font-weight:800;text-align:center" min="0" step="0.01"></div></div>' +
          ...
          <div class="w-72 space-y-2">
            ...
            <div class="flex justify-between text-sm items-center gap-2">
              ...
              <div class="flex items-center gap-1">
                ...
                <input type="number" inputmode="decimal" id="pb-discount" value="${f.discount_amount}" onchange="window._pb.updateDiscount()" class="border border-gray-300 rounded px-2 py-0.5 text-xs w-20 text-right" step="0.01">
              </div>
            </div>
  ...
  function renderItemRow(item, i) {
    ...
    return `<tr class="border-b border-gray-100">
      ...
      <td class="px-2 py-1"><input type="number" inputmode="decimal" value="${item.quantity}" onchange="window._pb.updateItem(${i},'quantity',this.value)" class="w-full border border-gray-200 rounded px-2 py-1.5 text-sm text-center" min="0" step="0.01"></td>
      ...
      <td class="px-2 py-1"><input type="number" inputmode="decimal" value="${item.unit_price}" onchange="window._pb.updateItem(${i},'unit_price',this.value)" class="w-full border border-gray-200 rounded px-2 py-1.5 text-sm text-right" min="0" step="0.01"></td>
        ...
        if (state.showAreaToCustomer) {
          ...
          sections += '<div class="p-8 border-b border-gray-200">' +
            ...
            '<div class="grid grid-cols-2 sm:grid-cols-4 gap-3">' +
              '<div class="text-center p-4 rounded-xl bg-gray-50 border border-gray-100"><p class="text-2xl font-black text-gray-800">' + Math.round(r.total_footprint_sqft || r.total_true_area_sqft || 0) + '</p><p class="text-xs text-gray-500 mt-1">Roof Area (sq ft)</p></div>' +
    ...
    openNewLibraryItemModal() {
      ...
      overlay.innerHTML = '<div style="background:white;border-radius:16px;padding:28px;max-width:420px;width:90%;box-shadow:0 20px 40px rgba(0,0,0,0.2)">' +
        ...
        '<div style="display:grid;gap:10px">' +
          ...
          '</div>' +
          '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">' +
            '<div><label style="font-size:11px;font-weight:600;color:#6b7280;display:block;margin-bottom:3px">Default Price ($)</label><input id="lib-price" type="number" inputmode="decimal" value="0" min="0" step="0.01" style="width:100%;padding:8px 10px;border:1px solid #d1d5db;border-radius:7px;font-size:13px;box-sizing:border-box"></div>' +
            '<div><label style="font-size:11px;font-weight:600;color:#6b7280;display:block;margin-bottom:3px">Default Qty</label><input id="lib-qty" type="number" inputmode="decimal" value="1" min="0" step="0.01" style="width:100%;padding:8px 10px;border:1px solid #d1d5db;border-radius:7px;font-size:13px;box-sizing:border-box"></div>' +
          '</div>' +
