...
// Floating "Verify in 3D" button — opens /3d-verify in a modal iframe so the
// surveyor can inspect the roof on Photorealistic 3D Tiles before/after tracing.
function mountTrace3DButton(mapDiv, getLatLng) {
  if (!mapDiv || mapDiv.dataset.rm3dMounted === '1') return;
  mapDiv.dataset.rm3dMounted = '1';

  // Make sure the host has positioned children
