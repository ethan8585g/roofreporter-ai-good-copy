...
// ============================================================
// REENGAGEMENT BLAST — One-shot promo email to inactive signups
// ============================================================
// Targets two cohorts in one query:
//   - never_ordered: signed up ≥3 days ago, zero orders ever
//   - dormant:       last order ≥60 days ago
// Per-recipient copy varies based on cohort (see buildReengagementHtml).
// Modes: ?mode=preview (count + sample), ?mode=send (actually fires).
// Pacing: sequential awaits — for ≤200 recipients, comfortably under
// the 30s worker CPU budget (Gmail send is mostly I/O which is uncounted).
adminRoutes.post('/marketing/reengagement-blast', async (c) => {
  const env: any = c.env
  ...
  if (mode === 'preview') {
    return c.json({
      ...
      mode: 'preview',
      counts,
      sample: recipients.slice(0, 10).map(r => ({ email: r.email, first_name: r.first_name, cohort: r.cohort, total_orders: r.total_orders })),
    })
  }

  if (mode !== 'send') {
    return c.json({ ok: false, error: 'mode must be preview or send' }, 400)
  }

  // --- Resolve Gmail OAuth2 creds (env, with D1 fallback for refresh_token)
  const clientId = env.GMAIL_CLIENT_ID || ''
  let clientSecret = env.GMAIL_CLIENT_SECRET || ''
  let refreshToken = env.GMAIL_REFRESH_TOKEN || ''
  if (!refreshToken) {
  ...
  await logAdminAction(c.env.DB, {
    ...
    action: 'reengagement_blast_sent',
    targetType: 'marketing_blast',
    after: { delivered: sent.length, failed: failed.length, dry_run: dryRun, counts },
    ip: clientIp(c),
  })
...
// HTML template for the reengagement blast — matches the brand's existing
// transactional emails (black header, white body, navy CTA). Plain enough
// to render in any client; no external CSS, no remote images other than
// the logo already used by report delivery emails.
