...
function buildSystemPrompt(
  ...
  return [
    ...
    '',
    'RULES:',
    '- 🔑 ROOF EDGE = TEXTURE TRANSITION (the most reliable rule). The eave is the pixel where organized roof texture (shingle pattern, metal seams, tile rows, asphalt grain) ends and yard / concrete / dirt / driveway begins. To place a vertex correctly, walk outward in pixel space from a confirmed-roof point until the texture changes — that pixel IS the eave. If a candidate vertex sits on irregular green (grass), smooth grey (concrete / asphalt), brown (dirt / mulch), or a fence/sidewalk line, YOU HAVE OVER-EXTENDED — pull the vertex back to the nearest texture boundary. This rule SUPERSEDES Solar API\'s rectangles and bbox when they disagree — Solar over-projects roof bounds, the visible texture transition does not.',
...
async function refineEavesViaSelfCritique(
  ...
  const bboxClause = args.targetBboxPx
    ? ((args.targetBboxPx as any).source === 'hint'
        ? `Target building bbox is AUTHORITATIVE from operator Mark Region (~${args.targetBboxPx.widthFt}ft × ${args.targetBboxPx.depthFt}ft); the first-draft polygon should cover all wings/extensions inside this region. Refinement may EXPAND (if visible roof extends beyond the draft) OR SHRINK (if the draft included yard / grass / driveway visible at any vertex). Trust the texture transition, not the bbox.`
  ...
  const critiqueSystem = [
    ...
    '',
    'Common OVER-EXTENSIONS to look for (these matter just as much as misses):',
    '- Polygon vertices landing on visible GRASS / YARD (irregular green texture, no organized roof pattern) — pull those vertices back to the nearest texture transition.',
    '- Polygon vertices landing on DRIVEWAY / SIDEWALK / CONCRETE / ASPHALT (smooth grey, no shingle / seam / tile pattern) — same fix.',
    '- Polygon following a Solar API rectangle into backyard or beyond the visible roof — Solar over-projects roof planes by 10-30ft; trust the visible roof edge over Solar.',
    '- Polygon merging with a neighbour across a visible gap (driveway, fence, lawn strip) — separate them into distinct polygons or drop the neighbour.',
    '',
    'Output rules:',
