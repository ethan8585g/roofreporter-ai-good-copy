    ...
    window.rmBlogLeadMagnetSubmit = async function(e) {
      ...
      try {
        ...
        if (res.ok && data && data.success !== false) {
          ...
          try { if (typeof window.fireMetaLeadEvent === 'function') { window.fireMetaLeadEvent({ content_name: '${fid}' }); } else if (typeof window.fbq === 'function') { window.fbq('track','Lead'); } } catch(_) {}
          return false;
        }
